//---------------------------------------------------------------------------
// $RCSfile: wxArchive.h $
// $Source: contrib/include/wx/common/wxArchive.h $
// $Revision: 1.18 $
...
public:
    /** Construct an input stream archive. The stream will be used to retrieve the data from, which can be any 
        ...
        if(a.IsOk())
        {
            ...
            wxLogError(_T("Archive cannot be opened because : ") + a.GetLastErrorString());
    ...
    */
    ...
    /** In reading mode, it returns true when the stream we are reading from returns true on Eof(). In writing mode Eof()
	    returns always false. When the stream is closed, true is always returned, we can't read or write anymore in that
    ...
    */
    ...
    /** Convenience operator for serializing. See WriteString() */
	virtual wxArchive &operator <<(const char* pvalue) {
    ...
    virtual wxArchive &operator <<(double value) { 
        WriteDouble(value); 
	    return *this; 
    };
    ...
    */
	...
	bool ReadDouble(double& value, double defval) {
    ...
    */
    ...
    /** Reads a record from the stream. The wxCharBuffer is a growable buffer class, which will contain the data 
		...
		There is no conversion taking place reading or writing characters from the buffer.
	*/
    bool Read(wxCharBuffer &buffer);

	/** Variant of Read() that will provide a default if the read was not succesful */
	bool Read(wxCharBuffer& value, const wxCharBuffer &defval) {
		bool val = Read(value);
		if(!val)
			value = defval;
		return val;
...
private:
	...
	int LogError(int err, int msgcode, const char *s1 = 0, const char *s2 = 0);
